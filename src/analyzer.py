...
keyword_extractor = KeyBERT(embedding_model)
...
try:
    ...
    nlp_processor.tokenizer = setup_custom_tokenizer(nlp_processor)
except OSError:
...
def generate_contextual_keywords(user_persona, user_task, challenge_metadata, top_n=20):
    """
    (Original Function) Extracts, ranks, and filters task-specific KEYPHRASES.
    """
    persona_role = user_persona['role']
    task_description = user_task['task']
    description = challenge_metadata.get('description', '')
    test_case_name = challenge_metadata.get('test_case_name', '')
    combined_query_text = (
    ...
    initial_keybert_phrases = keyword_extractor.extract_keywords(
        combined_query_text,
    ...
    query_embedding = embedding_model.encode(combined_query_text, convert_to_tensor=True, show_progress_bar=False)
    ranked_keywords_with_scores = []
    if initial_keywords_set:
        keyword_texts = list(initial_keywords_set)
        keyword_embeddings = embedding_model.encode(keyword_texts, convert_to_tensor=True, show_progress_bar=False)
...
def extract_simple_keywords(task_description):
    """
    (New Function) Extracts important SINGLE WORDS (nouns, verbs, adjectives) using spaCy.
    """
    # Process the text with spaCy
    doc = nlp_processor(task_description.lower())
...
def calculate_title_boost(title, phrase_keywords, simple_keywords):
    """
...
def format_section_title(paragraph):
    """
...
def calculate_weighted_score(query_embed, para, para_embed, phrase_keywords, simple_keywords, title_boost, filename_keyword_boost, is_veg_request, is_gluten_free_request):
    """
...
def analyze_persona_job(parsed_docs, persona, task, challenge_info, all_outlines_data, max_results=8):
    ...
    query_embed = embedding_model.encode(query, convert_to_tensor=True, show_progress_bar=False)

    # --- TIERED KEYWORD GENERATION ---
    phrase_keywords = generate_contextual_keywords(persona, task, challenge_info, top_n=30)
    simple_keywords = extract_simple_keywords(task['task'])
    ...
    all_section_embeddings = embedding_model.encode(all_section_texts, convert_to_tensor=True, show_progress_bar=True) 
    ...
    for i, section_data in enumerate(sections_to_process):
        ...
        current_title_boost = calculate_title_boost(final_title, phrase_keywords, simple_keywords)
        
        calculated_section_score = calculate_weighted_score(
