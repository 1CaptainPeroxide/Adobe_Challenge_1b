...
def extract_headers_with_pymupdf(doc_fitz):
    """
    ...
    header_sizes = {size for size in unique_sizes if size > most_common_size + 1}
            ...
            if "lines" in b:
                for l in b["lines"]:
                    if l["spans"]:
                        ...
                        if font_size in header_sizes:
                            line_text = "".join([s["text"] for s in l["spans"]]).strip()
                            if (len(line_text.split()) < 15 and
                                not line_text.endswith(('.', ':')) and
                                re.search('[a-zA-Z]', line_text) and
                                len(line_text) > 3):
                                if not temp_headers or temp_headers[-1][0] != line_text:
                                    temp_headers.append((line_text, page.number))
    i = 0
    while i < len(temp_headers):
        current_header, page_number = temp_headers[i]
        if (i + 1 < len(temp_headers) and 
            len(temp_headers[i+1][0].split()) < 4 and 
            temp_headers[i+1][0].islower()):
            current_header += " " + temp_headers[i+1][0]
            i += 1
        headers.append((current_header, page_number))
        i += 1
    return headers


def extract_document_title(doc_plumber):
...
def extract_headings_from_pdf(doc_plumber, doc_fitz):
    ...
    for page_idx, page in enumerate(doc_plumber.pages):
        ...
        table_texts = set(sanitize_text(cell) for table in tables for row in table if row for cell in row if cell)
        
        for i, line in enumerate(lines):
            clean_line = sanitize_text(line)
            if not clean_line or clean_line in table_texts: continue
            
            next_line = sanitize_text(lines[i+1]) if i+1 < len(lines) else ""
    ...
    pymupdf_headings = extract_headers_with_pymupdf(doc_fitz)
...
def process_pdf_file(pdf_path):
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as doc_plumber:
            ...
            # 1. Get Title
            title = extract_document_title(doc_plumber)
