...
text_summarizer = pipeline("summarization", model=model_name)
...
def rank_sections(matches, persona, task, max_total=6, max_per_document=2):
    ...
    for section in sorted_matches:
        ...
        if document_count[doc] >= max_per_document:
            continue
    ...
    refined_subsections = process_subsection_batch(subsections_to_refine)

    return output_sections, refined_subsections


def process_subsection_batch(subsections_data):
    """
    ...
    for item in subsections_data:
        item['cleaned_text'] = sanitize_output_text(item['text'])
